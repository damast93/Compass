...
Public Class ExpressionInterpreter : Implements Parser.ExpressionVisitor(Of Value)
    ...
    Public Function Intersect(A As Expression, B As Expression) As Value Implements ExpressionVisitor(Of Value).Intersect
        Dim _a = A.Accept(Me).AsOf(Of Geometry)()
        Dim _b = B.Accept(Me).AsOf(Of Geometry)()
    ...
    Public Function Point(x As Double, y As Double) As Value Implements ExpressionVisitor(Of Value).Point
        Return New Point(Unit * x, Unit * y)
    End Function

    Public Function NamePoint(name As String, pnt As Parser.Expression) As Value Implements ExpressionVisitor(Of Value).NamePoint
        Dim point = pnt.Accept(Me).AsOf(Of Point)()
        Return New NamedPoint(name, point.x, point.y)
    End Function

    Public Function [Sub](pars As IEnumerable(Of String), body As IEnumerable(Of Parser.Statement)) As Value Implements ExpressionVisitor(Of Value).Sub
    ...
    Public Function HalfRay(a As Parser.Expression, b As Parser.Expression) As Value Implements Parser.ExpressionVisitor(Of Value).HalfRay
        Dim _a = a.Accept(Me).AsOf(Of Point)()
        Dim _b = b.Accept(Me).AsOf(Of Point)()
