...
Public Class CanvasDisplay
    ...
    Private WithEvents m_Canvas As Canvas
    ...
    Public Overrides Sub RenderCircle(circle As Circle)
    ...
    Public Overrides Sub RenderPoint(point As Point)
        Dim pnt = New Ellipse()
        With pnt
    ...
    Public Overrides Sub RenderNamedPoint(point As NamedPoint)
        RenderPoint(point)
        Dim lbl = New Label()
        With lbl
            .Content = point.Name
    ...
    Public Overrides Sub RenderRay(ray As Line)
    ...
    Private Structure RayUpdater
        ...
        Public Sub Canvas_SizeChanged(sender As Object, e As System.Windows.SizeChangedEventArgs)
            Update()
        End Sub
        ...
        Public Sub Update()
            ...
            For Each seg In CanvasBounds
                ...
                For Each p In ints
                    Dim _P = p
                    If pts.TrueForAll(Function(x) _P.DistanceTo(x) > EPS) Then
                        pts.Add(p)
                    End If
                Next
            Next
        ...
        Public ReadOnly Property CanvasBounds As IEnumerable(Of Segment)
            Get
                ...
                Dim bounds As New List(Of Segment)
                bounds.Add(New Segment(NW, NE))
                bounds.Add(New Segment(NE, SE))
                bounds.Add(New Segment(SE, SW))
                bounds.Add(New Segment(SW, NW))
    ...
    Public Overrides Sub RenderSegment(r As Segment)
        Dim line As New WPFLine
        line.X1 = (r.A.x)
        line.Y1 = (r.A.y)
        line.X2 = (r.B.x)
    ...
    Public Overrides Sub RenderHalfRay(ray As Ray)
        Dim line As New WPFLine
